...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
    $scope.post = {};
    $scope.temp = {};
    $scope.Page = "ST_CLG_ROADMAP";
    $scope.editMode = false;
    $scope.FROMPAGE = 'ST';
    $scope.temp.roadmapid = 0;
    $scope.temp.regid = 0;
    $scope.temp.ddlStudentType = 'Registered';
    ...
    $scope.init = function () {
        ...
        then(function (data, status, headers, config) {
            ...
            if (data.data.success) {
                ...
                if($scope.LOCID > 0) $scope.temp.ddlLocation = $scope.LOCID.toString();
    ...
    $scope.getStudentCollegeRoadmap = function (roadmapid,regid) {
        ...
        $http({
             method: 'post',
             url: url,
            data: $.param({ 'type': 'getStudentCollegeRoadmap','roadmapid':0,'regid':$scope.userid}),
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
        }).
        then(function (data, status, headers, config) {
            // console.log(data.data);
            ...
            //  console.log($scope.post.getStudentCollegeRoadmap[0]);
             data.data.success ? $scope.editData($scope.post.getStudentCollegeRoadmap[0]) : $scope.setStudentDetails();
        },
        function (data, status, headers, config) {
            console.log('Failed');
        })
...
$scope.setStudentDetails = () => {
    ...
    // console.log($scope.post.getStudentByLoc);

    // alert($scope.temp.ddlStudent);
    // $timeout(()=>{
        $scope.$watch('temp.ddlStudent', function () {
            ...
            $scope.temp.txtClassof = ($scope.DETAILS[0]['CLASSOF'] && $scope.DETAILS[0]['CLASSOF']!='' && $scope.DETAILS[0]['CLASSOF']!='null') ? Number($scope.DETAILS[0]['CLASSOF']):''; 
...
$scope.getStudentByLoc = function () {
    ...
    then(function (data, status, headers, config) {
        ...
        if(data.data.success){
            ...
            $scope.temp.ddlStudent = $scope.userid.toString();
