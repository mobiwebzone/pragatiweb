...
function getTaskTracking($mysqli){
	try
	{
		...
		FROM TASK_TRACKING TT  WHERE ISDELETED=0 AND SSIGNEDTO_ID=$userid AND  LOCID=$ddlLocation ORDER BY  CONVERT(DATE,TTDATE,(106))  DESC";
...
function saveTaskTrackingDetails($mysqli){
	try
	{
	   ...
	   if($txtReview == '')throw new Exception("Please Enter Review.");
		   ...
		   $query="INSERT INTO TASK_TRACKING_DETAILS (TTID,REVIEWEDBYID,REVIEWEDON,REVIEW,SEND_BY,FILELINK,INSERTDATE)VALUES($TTID,$userid,GETDATE(),'$txtReview','STUDENTS','$txtLinkReview',$userid)";
