...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout,$window,taOptions) {
    ...
    var url = 'code/Assessment_code.php';
    ...
    $scope.viewPdfFullScreen = function(){
        window.open(`../backoffice/images/question_master/${$scope.SectionShowPdf}`,'_blank');
    }
    ...
    /* =============== CHECK SESSION ============== */
    $scope.init = function () {
        // const timer = document.querySelector('#mytimer');
        ...
        then(function (data, status, headers, config) {
            ...
            if (data.data.success) {
                ...
                $scope.getTestByStudentProducts();
            }
            else {
    ...
    $scope.AllSectionId = [];
    $scope.getTestByStudentProducts = function () {
        $('.lds-hourglass').show();
        $http({
            method: 'post',
            url: url,
            data: $.param({ 'type': 'getTestByStudentProducts'}),
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
        }).
        then(function (data, status, headers, config) {
            // console.log(data.data);
        ...
        function (error) {
            $('.lds-hourglass').hide();
            console.log(error);
            alert(`Error occurred : ${error.status} ${error.statusText}`);
            // location.reload();
            // console.log(data);
        })
...
//    $scope.getTestByStudentProducts(); --INIT
   ...
   /* ========== CHK/UNCHK ALL =========== */
    ...
    $scope.checkSelectAll = function(parent_index,index,testid,GROUPNO,TSECID,infoData){
        ...
        // console.log(infoData.filter(x => x.GROUPNO == GROUPNO && x.TSECID != TSECID));
        // console.info('GroupNo : ' + GROUPNO);
        ...
        if(Number(GROUPNO) > 0){
            ...
            // console.log('LENGTH : '+ $scope.TOTAL_GROUPDATA.length);
        }else{
            $scope.TOTAL_GROUPDATA = 0;
        }
        ...
        // console.log($scope.chkSectionForTest);
        $scope.SectionIDForTest = $scope.chkSectionForTest[parent_index.toString()];
        // console.log($scope.SectionIDForTest);

        if($scope.TOTAL_GROUPDATA.length>1 && !$scope.GROUP_CHECKED){
            // console.log('SAVE : ' + $scope.GROUP_CHECKED);
            $scope.GROUP_CHECKED =true;
            // console.log(infoData);
            // console.log(infoData.findIndex(x => x.TSECID == $scope.GROUPDATA[0]['TSECID']));
            ...
            // console.log($scope.SectionIDForTest);
            // $scope.SectionIDForTest.push();
            // alert();
        }else{
            // console.log('REMOVE : ' + $scope.GROUP_CHECKED);
            if($scope.TOTAL_GROUPDATA.length>1){
                $scope.GROUP_CHECKED = false;
                // console.log('REMOVE2 : ' + $scope.GROUP_CHECKED);
                // console.log(infoData);
                // console.log(infoData.findIndex(x => x.TSECID == $scope.GROUPDATA[0]['TSECID']));
                var gindex = infoData.findIndex(x => x.TSECID == $scope.GROUPDATA[0]['TSECID']);
        ...
        // $scope.CHECKIF_FORGROUP = false;
        if($scope.id >= 0 && $scope.id != testid){
            // $scope.CHECKIF_FORGROUP = true;
            // console.log('if0');
            if($scope.PI != undefined){
                // console.log('if1');
                // console.warn($scope.chkSectionForTest[$scope.PI]);
        ...
        // console.info('BOTTOM = ID : '+$scope.id+'//// TESTID : '+ testid +'//// PI : ' + $scope.PI);

            // for(t=0; t<$scope.chkSectionForTest.length; t++){
            //     $scope.chkSectionSelect = $scope.chkSectionForTest[t].reduce((a, b) => a + b, 0) > 0 ? true : false;
            // }
            // $scope.allChk = $scope.chkSectionForTest.toString() ===  $scope.AllSectionId.toString() ? '1' : '0';
            ...
            // console.log($scope.GROUP_CHECKED);
            // console.log('=====================================================');
    }
    /* ========== CHK/UNCHK ALL =========== */
   ...
   $scope.clear=function(){
    ...
    $scope.crosQuestion = function(pidx,idx){
        ...
        if (prevLineThrough) {
            $scope.post.getSectionQuestions[pidx]['STUDENTANS'] = '';
                ...
                then(function (data, status, headers, config) {
                    ...
                    if(data.data.success){
                        ...
                        if($scope.post.getTestSections.length > 0){
                            ...
                            $scope.SectionShowCalc = $scope.SectionsData['SHOWCALC'];
                            $scope.SectionShowPdf = $scope.SectionsData['PDFFILE'];
    ...
    $scope.SaveTest = function(){
        ...
        then(function (data, status, headers, config) {
            ...
            if (data.data.success) { 
                ...
                // alert(data.data.message);
                $scope.messageFailureToast(data.data.message);
    ...
    /*============ UPDATE ANSWER =============*/
    $scope.UpdateAnswer = function(RID, ans, sketchpadID, index){
        // console.log(sketchpadID);
        // if($scope.Testid > 0 && $scope.Tsecid > 0 && queid > 0 && ans != ''){
        if($scope.Testid > 0 && $scope.Tsecid > 0 && RID > 0){
            // console.log(sketchpadID)
            $scope.post.getSectionQuestions[index]['STUDENTANS']=ans.trim();
            ...
            alert('Somthing went wrong. Please refresh and try again.')
    ...
    $scope.changePage = function(N_P){
        if($scope.chunkData[$scope.NextPrevious-1]){
            $scope.post.getSectionQuestions=$scope.chunkData[$scope.NextPrevious-1];
            // console.log($scope.post.getSectionQuestions);
            $timeout(function(){
                window.location.hash = '#pageHead';
            },200);
        }
    ...
    $scope.startTimer = function(duration, display) {
        ...
        interval = $interval(function () {
            ...
            if(x>=10){
                x=0;
                // console.clear(); remove comment
            }else{
                x++;
            }
            // console.log(x, " : ", minutes , " : " , seconds); remove comment
    ...
    $scope.submitNextSecAlert = function(reply){
            if(reply == 'yes' && !$scope.BREAK) var allok =$scope.checkAnswers();
        // if(allok){
            if(reply == 'no'){
                $(".confirmNextSection").slideUp();
                $(".next-sec-submit-btn").removeAttr('disabled');
    ...
    $scope.checkAnswers = function() {
        var selectedQueNotAnswerd = [];
        var unansweredQuestions = $scope.post.getSectionQuestions.forEach(function(question,index) {
            // console.log(question.STUDENTANS);

            // console.log($scope.flagsList);

            if(!question.STUDENTANS || question.STUDENTANS == '' || question.STUDENTANS === null){
                // console.log(question)
                $scope.flags[index] = true;
                var fid = `Flag${index}`;
                var idx = $scope.flagsList.map(e => e.id).indexOf(fid);
                // console.log('if : '+idx);
                if(idx < 0){
                    var dt = {'id':fid,'que':`Q${index+1}`};
                    $scope.flagsList.push(dt);
                }
                selectedQueNotAnswerd.push(question);
            }else{
          ...
          return selectedQueNotAnswerd;
        });
        
        // SORT
        $scope.flagsList.sort(function(a, b) {
            const idA = parseInt(a.que.slice(1));
            const idB = parseInt(b.que.slice(1));
            return idA - idB;
        });

        // console.log(typeof selectedQueNotAnswerd);
        // console.log(selectedQueNotAnswerd);
        // console.log(selectedQueNotAnswerd.length);
        if(selectedQueNotAnswerd.length>0){
            $scope.warngMags = `You have not answered <b class='text-danger'>${selectedQueNotAnswerd.length}</b> Questions. Do you want to go back and fix or continue with the submission.`;
            $('#notAnsModal').modal('show');
            // var r = confirm(`You have not answered ${selectedQueNotAnswerd.length} Questions. Do you want to go back and fix or continue with the submission.`);
            return false;
        }
        else{
            return true;
    ...
    $scope.ClearAllData = function () { 
        ...
        $scope.SectionShowCalc = '';
        $scope.SectionShowPdf = '';
        $scope.unsaved = true;
        $scope.CheckTestStart = false;
        $scope.post.getSectionQuestions = [];
        $scope.TotalChunks = 0;
    ...
    /* ========== MESSAGE =========== */
    $scope.messageSuccessToast = function (msg) {
    ...
    $scope.messageFailureToast = function (msg) {
        jQuery('#myToast > .toast-body > samp').html(msg);
        jQuery('#myToast').toast('show');
