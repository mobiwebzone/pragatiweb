...
if( isset($_POST['type']) && !empty($_POST['type'] ) ){
	...
	switch ($type) {
        ...
        case "getScrollInfo":getScrollInfo($conn);break;
...
/*============ GET SCROLL INFO =============*/ 
function getScrollInfo($mysqli){
	try
	{
		$data = array();
		$query = "SELECT DIV_NO,INFO,INFO_LINK,SEQNO,COLOR,ISHEADER,HEADER
		FROM INDEX_INFO_SCROLL WHERE ISDELETED=0 AND
		CONVERT(DATE,GETDATE(),105) BETWEEN CONVERT(DATE,DISPLAY_FROM,105) AND CONVERT(DATE,DISPLAY_TO,105)
		ORDER BY DIV_NO,SEQNO";
		$count = unique($query);
		if($count>0){
			$result = sqlsrv_query($mysqli, $query);
			$DIV_NO_NEW=$DIV_NO_OLD=0;
			// $data['SCROLLS'] = array();
			$TEXT = '';
			while ($row = sqlsrv_fetch_array($result,SQLSRV_FETCH_ASSOC)) {
				$row['DIV_NO'] = (int) $row['DIV_NO'];
				$ISHEADER = $row['ISHEADER'];
				$HEADER = $row['HEADER'];
				$COLOR = $row['COLOR'];
				$INFO = $row['INFO'];
				$INFO_LINK = $row['INFO_LINK'];
				$DIV_NO_NEW = $row['DIV_NO'];

				
				if($DIV_NO_NEW!=$DIV_NO_OLD){
					$TEXT = '';
					if($ISHEADER==1){
						// $TEXT .= "<hr class='my-2 mb-0'>";
						if(!empty($HEADER) && $HEADER!='') $TEXT .= "<h4 class='mb-1'>$HEADER</h4>";
					}
					if(empty($INFO_LINK) || $INFO_LINK==''){
						$TEXT .= "<div class='my-2' style='color:$COLOR'>$INFO</div>";
					}else{
						$TEXT .= "<div class='my-2'><i style='color:#343fff' class='fa fa-link'></i> <a title='click here to open' style='color:$COLOR' href='$INFO_LINK' target='_blank'>$INFO</a></div>";
					}
				}else{
					// $TEXT .='<hr class="my-2">';
					if($ISHEADER==1){
						$TEXT .= "<hr class='my-2 mb-0'>";
						if(!empty($HEADER) && $HEADER!='') $TEXT .= "<h4 class='mb-1 font-weight-bold text-monospace'>$HEADER</h4>";
					}
					if(empty($INFO_LINK) || $INFO_LINK==''){
						$TEXT .= "<div class='my-2' style='color:$COLOR'>$INFO</div>";
					}else{
				...
				$data['SCROLLS'][$DIV_NO_NEW]['TEXT'] = $TEXT;
				$data['data'][] = $row;
				$DIV_NO_OLD = $DIV_NO_NEW;
			}
			$data['success'] = true;
		}else{
			$data['success'] = false;
