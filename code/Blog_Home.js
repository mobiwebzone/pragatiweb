...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout,$sce) {
    $scope.post = {};
    $scope.temp = {};
    $scope.PAGE = "BLOG_HOME";
    $scope.editMode = false;
    $scope.trustAsHtml = $sce.trustAsHtml;
    $scope.PAGE_INSIDE = false;

    var url = 'code/Blog_Home.php';
    ...
    $scope.getBlogs = function (f) {
        if(f!='') jQuery('.btn-reload i').addClass('fa-spin');
        ...
        then(function (data, status, headers, config) {
            ...
            if(f!='') jQuery('.btn-reload i').removeClass('fa-spin');
        },
        function (data, status, headers, config) {
            console.log('Failed');
        })
