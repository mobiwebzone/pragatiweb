...
function getTestReview($mysqli){
	try
	{
		...
		if($assfor == 'SECTION'){
			...
			STID IN ((SELECT STID FROM STUDENT_TESTS_FREE WHERE ISDELETED=0 AND ATTEMPT=$attempt AND REGID=$userid AND TESTID=$testid AND 
			TSECID IN (SELECT TSECID FROM TEST_SECTION_GROUPS WHERE ISDELETED=0 AND TESTID=$testid AND GROUPNO=$assgroupno)))
			AND REGID=$userid 
			AND TSECID IN ((SELECT TSECID FROM STUDENT_TESTS_FREE WHERE ISDELETED=0 AND ATTEMPT=$attempt AND REGID=$userid AND TESTID=$testid AND 
			TSECID IN (SELECT TSECID FROM TEST_SECTION_GROUPS WHERE ISDELETED=0 AND TESTID=$testid AND GROUPNO=$assgroupno)))";
		}
