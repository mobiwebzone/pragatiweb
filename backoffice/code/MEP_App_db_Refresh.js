...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
  ...
  $scope.PageSub1 = "MEPAPPDBREFRESH";
  $scope.temp.txtAppDbBackupDate = new Date();

  var url = "code/MEP_App_db_Refresh_code.php";
  ...
  $scope.save = function () {
    ...
    $http({
      ...
      transformRequest: function (data) {
        ...
        formData.append("txtAppDbBackupDate", $scope.temp.txtAppDbBackupDate.toLocaleString('sv-SE'));
        formData.append("txtAppDBTypeId", $scope.temp.txtAppDBTypeId);
        formData.append("txtMainServerLocationId", $scope.temp.txtMainServerLocationId);
        formData.append("txtBackupServerLocationId", $scope.temp.txtBackupServerLocationId);
        formData.append("txtMainServerObjCount", $scope.temp.txtMainServerObjCount);
        formData.append("txtBackServerObjCount", $scope.temp.txtBackServerObjCount);
  ...
  $scope.getQuery = function () {
   ...
   //Get Backup Type
  $scope.getAppDBType = function () {
    $scope.post.getAppDBType = [];
    ...
    $http({
      ...
      function (data, status, headers, config) {
        console.log(data.data);
        $scope.post.getAppDBType = data.data.success ? data.data.data : [];
        $(".SpinBank").hide();
      },
      function (data, status, headers, config) {
        console.log("Failed");
  ...
  $scope.edit = function (id) {
    ...
    $scope.temp = {
      pmid: id.APPDBREFRESH_ID,
      txtTechPlatformId: id.TECHPLATFORMID.toString(),
      txtAppDbBackupDate: id.APPDB_BACKUP_DATE ? new Date(id.APPDB_BACKUP_DATE) : '',
      txtAppDBTypeId: id.APP_DB_TYPE_ID.toString(),
      txtMainServerLocationId: id.MAIN_SERVER_LOCATION_ID.toString(),
      txtBackupServerLocationId: id.BACKUP_SERVER_LOCATION_ID.toString(),
      txtMainServerObjCount: id.MAIN_SERVER_OBJECTS_COUNT.toString(),
      txtBackServerObjCount: id.BACKUP_SERVER_OBJECTS_COUNT.toString(),
  ...
  $scope.delete = function (id) {
    ...
    if (r == true) {
      $http({
        method: "post",
        url: url,
        data: $.param({ pmid: id.APPDBREFRESH_ID, type: "delete" }),
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
      }).then(function (data, status, headers, config) {
        console.log(data.data);
        if (data.data.success) {
