...
if( isset($_POST['type']) && !empty($_POST['type'] ) ){
	...
	switch ($type) {
        ...
        case "saveAmount":saveAmount($conn);break;
...
function getExpenseBudget($mysqli){
	try
	{
		...
		$query = "EXEC [FY_EXP_BUDGET_ACTUAL_TRACKING]$ddlLocation,$txtYear";
		$data['$query']=$query;
...
function saveAmount($mysqli){
	...
	$insertData = "UPDATE FY_EXPENSE_BUDGET_ACTUAL_TRACKING SET AMOUNT=$VAL,UPDATEID=$userid,UPDATEDATE=GETDATE() 
				WHERE EHID=$EHID AND LOCID=$ddlLocation AND FY=$txtYear AND MON=$MONTH";
	...
	if(!$stmt){
		...
		$data['success'] = true;
		$data['message'] = 'Amount successfully update.';			
	}
