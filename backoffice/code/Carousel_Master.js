...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
    ...
    $scope.Page = "SETTING";
    $scope.PageSub = "CAROUSEL_MASTER";
    $scope.files = [];
    $scope.imageDimensionIsCorrect = false;
    $scope.imageHeight = 0;
    $scope.imageWidth = 0;
    
    var url = 'code/Carousel_Master.php';
    ...
    $scope.UploadImage = function (element) {
        ...
        reader.onload = function (event) {
            ...
            if (element.files[0].type.startsWith('image')) {
                ...
                $scope.imageDimensionIsCorrect = (img.width==1920) ? false : true;
    ...
    $scope.save = function(){
     ...
     $scope.getCarouselMaster = function () {
        ...
        $http({
            method: 'post',
            url: url,
            data: $.param({ 'type': 'getCarouselMaster'}),
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
        }).
        then(function (data, status, headers, config) {
            console.log(data.data);
    ...
    $scope.edit = function (id) {
        ...
        $scope.Img_src= id.PIC != '' ? 'images/carousel/'+id.PIC : '';
        $scope.editMode = true;
    ...
    $scope.delete = function (id) {
        ...
        if (r == true) {
            $http({
                method: 'post',
                url: url,
                data: $.param({ 'CID': id.CID, 'type': 'delete' }),
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            }).
		    then(function (data, status, headers, config) {
                // console.log(data.data)
