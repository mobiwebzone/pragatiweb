 ...
 */
 ...
 function save($mysqli){
     try
     {
    	...
    	$TEXT_TASK_STATUS_CD  = ($_POST['TEXT_TASK_STATUS_CD'] == 'undefined' || $_POST['TEXT_TASK_STATUS_CD'] == '') ? '' : $_POST['TEXT_TASK_STATUS_CD'];
        $remarks = ($_POST['remarks'] == 'undefined' || $_POST['remarks'] == '') ? '' : $_POST['remarks'];

		$actionid = $pmid == 0 ? 1 : 2;
		
		$sql = "SELECT * FROM ISSUES_LIST 
		        WHERE TASK_ID!=$pmid 
				AND   ORG_ID = $TEXT_ORG_ID
				AND   PROJECT_ID = $TEXT_PROJECT_ID
				AND   ASSIGNED_TO_ID =$TEXT_ASSIGNED_TO_ID 
		        AND   CONVERT(DATE,STARTDATE,105)='$txtStartDT' 
		    ...
		    if ($txtEndDT != '' && $TEXT_TASK_STATUS_CD == '') {
				throw new Exception('Please change Issue Status to close!');
			}
			
			if ($txtEndDT == '' && $TEXT_TASK_STATUS_CD == 15) {
				throw new Exception('Please update Issue closing date!');
			}
			
			if($txtStartDT == '')
			{
				throw new Exception("Please Enter Issue Opening Date.");
			}
		...
		if($row_count == 0)
		{
			$query="EXEC [ISSUES_LIST_SP] $actionid,$pmid,$TEXT_ORG_ID,$TEXT_PROJECT_ID, $TEXT_TASK_CAT_ID, $TEXT_TASK_SUB_CAT_ID, $TEXT_ASSIGNED_TO_ID,
			    '','$txtStartDT','$txtEndDT','$txtTaskDesc','$TEXT_TASK_STATUS_CD','$remarks',$userid ";
 ...
 function getQuery($mysqli){
	try
	{
        ...
        $TEXT_ASSIGNED_TO_ID_S = ($_POST['TEXT_ASSIGNED_TO_ID_S'] =='undefined' || $_POST['TEXT_ASSIGNED_TO_ID_S'] =='') ? 0 : $_POST['TEXT_ASSIGNED_TO_ID_S'];
	   ...
	   $query =    " SELECT 
		...
		if ($TEXT_ASSIGNED_TO_ID_S != '') {
				$query .= " AND ASSIGNED_TO_ID = $TEXT_ASSIGNED_TO_ID_S "; 
...
function getAssignedToUser($mysqli){
 ...
 function getTaskstatus($mysqli){
	try
	{
        ...
        $query = "SELECT CODE_DETAIL_ID, CODE_DETAIL_DESC  FROM MEP_CODE_DETAILS WHERE ISDELETED=0  AND CODE_ID=6 ORDER BY CODE_DETAIL_ID";
...
function delete($mysqli){
	try{   
		...
		$data = array();     
		$pmid  = ($_POST['pmid'] == 'undefined' || $_POST['pmid'] == '') ? 0 : $_POST['pmid'];  
		$query = "EXEC [TASK_USERS_SP] 3,$pmid,'','','','','','','','','','',$userid,'' ";
		$data['$query'] = $query;
		$stmt=sqlsrv_query($mysqli,$query);
		if( $stmt === false ) 
		{
