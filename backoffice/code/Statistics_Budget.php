...
function getStatisticsBudget($mysqli){
	try
	{
		...
		$query = "EXEC [FY_STA_BUDGET]$ddlLocation,$txtYear,$NoOfYear,$txtFactor";
		$data['$query']=$query;
...
function saveBudget($mysqli){
	...
	$insertData = "UPDATE FY_STATISTICS_BUDGET SET BUDGET=$VAL,UPDATEID=$userid,UPDATEDATE=GETDATE() 
				WHERE SHID=$SHID AND LOCID=$ddlLocation AND FY=$txtYear AND MON=$MONTH AND NOOFYEAR=$NOOFYEAR";
