...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
    ...
    $scope.daysInWeek_num = [1, 2, 3, 4, 5, 6, 7];

    var url = 'code/Student_Analysis_DW_Chart.php';
    ...
    then(function (data, status, headers, config) {
        ...
        if(DATA_LEN>0){
            ...
            for($d=1;$d<=$scope.daysInWeek_num.length;$d++){
                $scope.DATES=$scope.post.getReport[$d]['DATE'];
                // console.log($scope.DATES);
                $scope.TOTALS=$scope.post.getReport[$d]['TOTAL'];
                $scope.AVEGS=$scope.post.getReport[$d]['AVEG'];
                
                // container = document.querySelector('#myCanvas')

                // CREATE HEADING
                var head = document.createElement('h3');
                head.innerHTML='<u>'+$scope.daysInWeek[$d-1].toUpperCase()+'</u>';
