 ...
 */
 ...
 function save($mysqli){
	...
	$sql = "SELECT * FROM ISSUES_MAIN_CATEGORIES 
	        WHERE TASK_CAT   ='$txtObject' 
			AND   TASK_CAT_ID!=$pmid 
		...
		if($row_count == 0)
		{
			$query="EXEC [ISSUES_MAIN_CATEGORIES_PM] $actionid,$pmid,'$TEXT_ORG_ID','$TEXT_PROJECT_ID','$txtObject','$txtremarks',$userid ";
...
function delete($mysqli){
	try{   
            ...
            $pmid = ($_POST['pmid'] == 'undefined' || $_POST['pmid'] == '') ? 0 : $_POST['pmid'];  
			if($pmid == 0){
				throw new Exception('TASK_CAT_ID Error.');
			}

			$stmt=sqlsrv_query($mysqli, "EXEC [ISSUES_MAIN_CATEGORIES_PM] 3,$pmid,'','','','',$userid ");
			if( $stmt === false ) 
			{
