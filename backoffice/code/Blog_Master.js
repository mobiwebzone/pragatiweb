...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
    ...
    $scope.Page = "MISC";
    $scope.PageSub = "BLOG";
    $scope.PageSub1 = "BLOG_MASTER";
    $scope.temp.txtLastUpdateDT = new Date();
    
    var url = 'code/Blog_Master.php';
    ...
    $scope.saveData = function(){
        ...
        $http({
            ...
            transformRequest: function (data) {
                ...
                formData.append("bcatid", $scope.temp.bcatid);
                formData.append("txtBlogCategory", $scope.temp.txtBlogCategory);
                return formData;
            },
            data: $scope.temp,
    ...
    $scope.editForm = function (id) {
        if($scope.userrole == 'ADMINISTRATOR' || $scope.userrole == 'SUPERADMIN'){
            $("#txtBlogCategory").focus();
            $scope.temp.bcatid = id.BCATID;
            $scope.temp.txtBlogCategory = id.BLOG_CATEGORY;
            $scope.editMode = true;
            $scope.index = $scope.post.getBlogCategories.indexOf(id);
        }
    }
    /* ============ Edit Button ============= */ 
        ...
        if($scope.userrole == 'ADMINISTRATOR' || $scope.userrole == 'SUPERADMIN'){
            ...
            if (r == true) {
                $http({
                    method: 'post',
                    url: url,
                    data: $.param({ 'BCATID': id.BCATID, 'type': 'delete' }),
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
                }).
                then(function (data, status, headers, config) {
                    // console.log(data.data)
                    if (data.data.success) {
                        var index = $scope.post.getBlogCategories.indexOf(id);
                        $scope.post.getBlogCategories.splice(index, 1);
