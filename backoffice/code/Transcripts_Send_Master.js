...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
    ...
    $scope.PageSub1 = "TSCRIPT_SEND_MASTER";
    $scope.editMode = false;
    
    var url = 'code/Transcripts_Send_Master.php';
    ...
    $scope.saveData = function(){
        ...
        $http({
            ...
            transformRequest: function (data) {
                ...
                formData.append("tsendid", $scope.temp.tsendid);
                formData.append("txtSendMethod", $scope.temp.txtSendMethod);
    ...
    $scope.editData = function (id) {
        ...
        $scope.temp = {
            tsendid:id.TSENDID,
            txtSendMethod:id.TSENDMETHOD,
            txtDesc:id.TSENDDESC,
            txtLink:id.TSENDLINK,
            txtComments:id.COMMENTS
        };

        $scope.editMode = true;
        $scope.index = $scope.post.getTranscriptSends.indexOf(id);
    }
    /* ============ Edit Button ============= */ 
    ...
    $scope.deleteData = function (id) {
        ...
        if (r == true) {
            $http({
                method: 'post',
                url: url,
                data: $.param({ 'TSENDID': id.TSENDID, 'type': 'deleteData' }),
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            }).
		    then(function (data, status, headers, config) {
                // console.log(data.data)
		        if (data.data.success) {
		            var index = $scope.post.getTranscriptSends.indexOf(id);
		            $scope.post.getTranscriptSends.splice(index, 1);
