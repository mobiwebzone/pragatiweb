    ...
    $scope.saveUsers = function(){
        ...
        $http({
            ...
            transformRequest: function (data) {
                ...
                formData.append("type", 'saveUsers');
                formData.append("uid", $scope.temp.uid);
                formData.append("txtFName", $scope.temp.txtFName);
                formData.append("txtLName", $scope.temp.txtLName);
     ...
     $scope.getUsers = function () {
        ...
        then(function (data, status, headers, config) {
            // console.log(data.data);
            $scope.post.getUser = data.data.data;
        },
        function (data, status, headers, config) {
            console.log('Failed');
        })
    ...
    $scope.deleteUsers = function (id) {
        ...
        if (r == true) {
            $http({
                method: 'post',
                url: url,
                data: $.param({ 'uid': id.UID, 'type': 'deleteUsers' }),
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            }).
		    then(function (data, status, headers, config) {
                // console.log(data.data)
		        if (data.data.success) {
		            var index = $scope.post.getUser.indexOf(id);
		            $scope.post.getUser.splice(index, 1);
