...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout,$window) {
    ...
    var url = 'code/Student_Attendance_Receipt_Report.php';
    ...
    then(function (data, status, headers, config) {
        // console.log(data.data);
        $scope.post.AttendanceData = data.data.successAtt?data.data.data_attendance:[];
        $scope.post.ReceiptsData = data.data.successRec?data.data.data_receipts:[];
        $scope.TOTAL_AMT = data.data.successRec?data.data.TOTAL_AMT:0;
        // $timeout(()=>{$scope.filterData()},1000);

        if(!data.data.successAtt) $scope.messageFailure(data.data.messageAtt);
        if(!data.data.successRec) $scope.messageFailure(data.data.messageRec);
        ...
        function (data, status, headers, config) {
            console.log('Failed');
            $scope.post.AttendanceData=[];
            $scope.post.ReceiptsData =[];
            $scope.TOTAL_AMT=0;
        })
    ...
    $scope.onPrintAtt = true;
    $scope.print = function(FOR){
        $('.printbtn').attr('disabled','disabled');
        $scope.onPrintAtt = FOR == 'REC' ? false : true;
        $timeout(()=>{
            $window.print();
