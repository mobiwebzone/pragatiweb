		...
		try
	{
		...
		$TEXT_RTE_CD  = $_POST['TEXT_RTE_CD'] == 'undefined' ? 0 : $_POST['TEXT_RTE_CD'];
						...
						$query = " SELECT 
										...
										SUM(A.Fees_due) AS TOTAL_FEES_DUE,
										SUM(A.fees_paid) AS TOTAL_FEES_PAID,
										A.class_cd,
										A.class
									...
									AND B.RTE_CD = $TEXT_RTE_CD
									GROUP BY A.class_cd, A.class
									UNION ALL
									SELECT 
										ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS ROWID,
										SUM(A.FEES_DUE) AS TOTAL_FEES_DUE,
						...
						else
						{
								$query = "SELECT 
									ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS ROWID,
									SUM(Fees_due) AS TOTAL_FEES_DUE,
								...
								SELECT 
									...
									SUM(Fees_due) AS TOTAL_FEES_DUE,
									SUM(fees_paid) AS TOTAL_FEES_PAID,
									100 AS class_cd,  -- Use NULL or an integer value for the class_cd
									'All Classes' AS class
								FROM STUDENT_FEES_MASTER 
								WHERE isdeleted = 0 
								AND school_id = $TEXT_SCHOOL_ID
