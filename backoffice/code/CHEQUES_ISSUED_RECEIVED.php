     ...
     try
     {
		...
		if(isset($_FILES['txtSignature']['name']) && $_FILES['txtSignature']['size'] > 0)
		{
			...
			else
			{
                ...
                {
                    move_uploaded_file($_FILES["txtSignature"]["tmp_name"], '../cheqIssRec_images/'.$txtSignature);
...
function getChequeDetails($mysqli){
	try
	{
		...
		(SELECT LOCID FROM BANKS WHERE BANKID=ISD.BANKID)LOCID,
        ...
        FROM CHEQUES_ISSUED_RECEIVED ISD WHERE ISDELETED=0 AND BANKID IN (SELECT BANKID FROM BANKS WHERE LOCID=$ddlLocation) ORDER BY CHDATE DESC";
