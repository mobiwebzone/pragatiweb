...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
    ...
    $scope.init = function () {
        ...
        then(function (data, status, headers, config) {
            ...
            if (data.data.success) {
                $scope.navMenu = data.data.finalData['RETURN_DATA'];
    ...
    $scope.getLocations = function () {
        ...
        then(function (data, status, headers, config) {
            ...
            $scope.temp.ddlLocationTotal = ($scope.post.getLocations) ? $scope.locid.toString():'';
            ...
            if ($scope.temp.ddlLocationTotal > 0) $scope.getDuplicateStudent();
             if($scope.temp.ddlLocationTotal > 0) $scope.getITDashBoard();
            if($scope.temp.ddlLocationTotal > 0) $scope.getTeacher_NO_ATT();
            if($scope.temp.ddlLocationTotal > 0) $scope.getStudentAccountDeleteRequest();
...
//    $scope.getDiscontinueReq(); --INIT
   ...
   $scope.DiscontinueApprove = function (x) {
    ...
    $scope.getStudentAccountDeleteRequest= function () {
    // alert($scope.REGID);
        $scope.spin_ST_DEL_REQ=true;
        if($scope.userrole != 'TSEC_USER'){
            $scope.post.getStudentAccountDeleteRequest=[];
            $http({
                method: 'post',
                url: url,
                data: $.param({ 'type': 'getStudentAccountDeleteRequest','ddlLocation':$scope.temp.ddlLocationTotal }),
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            }).
            then(function (data, status, headers, config) {
                console.log(data.data);
                if(data.data.success){
                    $scope.post.getStudentAccountDeleteRequest=data.data.data;
                }else{
                    // console.info(data.data.message);
                }
                $scope.spin_ST_DEL_REQ=false;
            },
            function (data, status, headers, config) {
                console.log('Failed');
            })
    ...
    // $scope.getStudentAccountDeleteRequest(); --INIT
    /*============ STUDENT ACCOUNT DELETE REQUEST LIST =============*/
   ...
   $scope.deleteAccount = function (x) {
    ...
    if($scope.userrole != 'TSEC_USER'){
        ...
        if (r == true) {
            ...
            $http({
                method: 'post',
                url: url,
                data: $.param({'type': 'deleteAccount','REGID': x.USERID,'ADRID':x.ADRID}),
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            }).
            then(function (data, status, headers, config) {
                // console.log(data.data)
    ...
    /*============ Get IT DashBoard =============*/
    $scope.getITDashBoard = function () {
    
        $('.spinDuplicateST').show();
       
        if ($scope.userrole != 'TSEC_USER') {
            $scope.post.getITDashBoard=[];
            $http({
                method: 'post',
                url: url,
                data: $.param({
            ...
            then(function (data, status, headers, config) {
                // console.log(data.data);
                if(data.data.success){
                    $scope.post.getITDashBoard=data.data.data;
                }else{
                     console.info(data.data.message);
                }
                $('.spinDuplicateST').hide();
    ...
    $scope.getITDashBoard(); 
    /*============ IT DashBoard =============*/

    $scope.getGlobalTime = function () {
        $scope.spinGtime = true;
        $http({
            method: 'post',
            url: url,
            data: $.param({
        ...
        then(function (data, status, headers, config) {
            // console.log(data.data);
            if(data.data.success){
                $scope.post.getGlobalTime=data.data.time;
            }else{
                    console.info(data.data.message);
            }
            $scope.spinGtime = false;
