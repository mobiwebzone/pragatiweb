 ...
 */
 ...
 function save($mysqli){
     try
     {
        ...
        $teacherclassesid  = ($_POST['teacherclassesid'] == 'undefined' || $_POST['teacherclassesid'] == '') ? 0 : $_POST['teacherclassesid'];
		...
		$actionid = $teacherclassesid == 0 ? 1 : 2;

		
		
				$sql = "SELECT * FROM TEACHER_CLASSES
		        WHERE TEACHER_CLASSES_ID!=$teacherclassesid
	   ...
	   if($row_count == 0)
	   {
		...
		$query="EXEC [Teacher_Class_Mapping_SP] $actionid,$teacherclassesid,$TEXT_SCHOOL_ID,$TEXT_CLASS_CD,$TEXT_TEACHER_ID,$userid,'$txtremarks' ";
 ...
 function getQuery($mysqli){
	...
	$TEXT_SCHOOL_ID  = $_POST['TEXT_SCHOOL_ID'] == 'undefined' ? 0 : $_POST['TEXT_SCHOOL_ID'];	
       ...
       $query =     "SELECT 
					 A.TEACHER_CLASSES_ID
					,A.TEACHER_ID
					,A.SCHOOL_ID
					,A.CLASS_CD
					,C.CODE_DETAIL_DESC CLASS
					,B.TEACHER_NAME
					FROM TEACHER_CLASSES A , TEACHER B ,MEP_CODE_DETAILS C
					WHERE A.TEACHER_ID = B.TEACHER_ID
					AND   A.CLASS_CD   = C.CODE_DETAIL_ID
					AND   A.ISDELETED =0
					AND   B.ISDELETED =0
					AND   C.ISDELETED =0 
		...
		while ($row = sqlsrv_fetch_array($result,SQLSRV_FETCH_ASSOC)) {
			$row['TEACHER_CLASSES_ID'] = (int) $row['TEACHER_CLASSES_ID'];
			$data['data'][] = $row;
		}
		$data['success'] = true;
		echo json_encode($data);exit;
...
function delete($mysqli){
	try{   
			...
			if($teacherclassesid == 0){
				throw new Exception('TEACHER_CLASSES_ID Error.');
			}
			
	
				$stmt=sqlsrv_query($mysqli, "EXEC [Teacher_Class_Mapping_SP]	3,$teacherclassesid,'','','',$userid,'' ") ;
