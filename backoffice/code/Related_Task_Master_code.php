...
function saveMain($mysqli){
	...
	//    if($ddlMainPriority == '')throw new Exception("Please Select Priority.");
	   ...
	   $sql = "SELECT * FROM TASK_MAIN_CATEGORIES_R WHERE LOCID=$ddlLocation AND TASKMAINCAT='$txtTaskMainCat' AND TASKMAINCATID!=$TASKMAINCATID AND ISDELETED=0";
	   $row_count = unique($sql);
	   
	   
	   if($row_count == 0)
	   {
		   $query="EXEC [TASK_MAIN_CATEGORIES_SP_R] $actionid,$TASKMAINCATID,$ddlLocation,'$txtTaskMainCat','$ddlMainPriority',$userid";
		   // $data['$query'] = $query;
...
function getTaskMainCategory($mysqli){
   try
   {
	   ...
	   $query = "SELECT TASKMAINCATID,LOCID,(SELECT [LOCATION] FROM LOCATIONS WHERE LOC_ID=GM.LOCID)[LOCATION],TASKMAINCAT,PRIORITY 
			   FROM TASK_MAIN_CATEGORIES_R GM
...
function deleteMain($mysqli){
   try{   
		...
		$query = "EXEC [TASK_MAIN_CATEGORIES_SP_R] 3,$TASKMAINCATID,0,'','',$userid";
		$data['$query'] = $query;
		$stmt=sqlsrv_query($mysqli,$query);
		if( $stmt === false ) 
		{
...
/*============ SAVE =============*/ 
 function save($mysqli){
     try
     {
		...
		$sql = "SELECT * FROM TASK_CATEGORIES_R WHERE TASKMAINCATID=$TASKMAINCATID AND LOCID=$ddlLocation AND TASKCAT='$txtTaskCAt' AND TASKMAINCATID=$TASKMAINCATID AND TASKCATID!=$TASKCATID AND ISDELETED=0";
		$row_count = unique($sql);
		
		
		if($row_count == 0)
		{
			$query="EXEC [TASK_CATEGORIES_SP_R] $actionid,$TASKCATID,$TASKMAINCATID,$ddlLocation,'$txtTaskCAt','$ddlPriority',$userid";
			// $data['$query'] = $query;
			// echo json_encode($data);exit;
			$stmt=sqlsrv_query($mysqli, $query);
...
function delete($mysqli){
	try{   
			...
			$data = array();     
            $TASKCATID = ($_POST['TASKCATID'] == 'undefined' || $_POST['TASKCATID'] == '') ? 0 : $_POST['TASKCATID'];  
			$query = "EXEC [TASK_CATEGORIES_SP_R] 3,$TASKCATID,0,0,'','',$userid";
			$data['$query'] = $query;
			$stmt=sqlsrv_query($mysqli,$query);
			if( $stmt === false ) 
			{
