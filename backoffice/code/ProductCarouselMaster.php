 ...
 */
 ...
 function save($mysqli){
     try
     {
        ...
        $pcid  = ($_POST['pcid'] == 'undefined' || $_POST['pcid'] == '') ? 0 : $_POST['pcid'];
		...
		$actionid = $pcid == 0 ? 1 : 2;
		if($ddlLocation == 0 && $ddlProduct == 0)throw new Exception("Please Select Location or Product Name.");
		...
		$sql = "SELECT * FROM PRODUCT_CAROUSEL_DISPLAY WHERE SEQNO='$txtSeqno' AND LOCID=$ddlLocation AND PDMID=$ddlProduct AND PCID!=$pcid AND ISDELETED=0";
		$row_count = unique($sql);

		if($row_count == 0)
		{
			$query="EXEC [PRODUCT_CAROUSEL_DISPLAY_SP] $actionid,$pcid,$ddlLocation,$ddlProduct,'$ddlDisplayType','$pictureUpload','$txtCaption','$txtValidFrom','$txtValidTo',$txtInterval,$txtSeqno,$userid";
			...
			else
			{
				...
				if($pictureUpload != ''){
					...
					if(isset($_FILES['pictureUpload']['name']) && $_FILES['pictureUpload']['size'] > 0)
					{
						move_uploaded_file($_FILES["pictureUpload"]["tmp_name"], '../images/product_carousel/'.$pictureUpload);
					}
	
					
					if(isset($_FILES['pictureUpload']['name']) && $existingPictureUpload != '')
					{
						if (file_exists('../images/product_carousel/'.$existingPictureUpload))
						{
							unlink('../images/product_carousel/'.$existingPictureUpload);
						}
					}
				}
...
/*============ GET CAROUSEL MASTER =============*/ 
 function getCarouselMaster($mysqli){
	try
	{
		...
		$ddlLocation = ($_POST['ddlLocation'] == 'undefined' || $_POST['ddlLocation'] == '') ? 0 : $_POST['ddlLocation'];
		$ddlProduct = ($_POST['ddlProduct'] == 'undefined' || $_POST['ddlProduct'] == '') ? 0 : $_POST['ddlProduct'];
		if($ddlProduct == 0 && $ddlLocation == 0) throw new Exception('Invalid PDMID OR LOCID.');
		$query = "SELECT PCID,DISPLAY_TYPE,PIC,PIC_CAPTION,LOCID,PDMID,
				CONVERT(VARCHAR,PIC_FROMDT,106)VALID_FROM,CONVERT(VARCHAR,PIC_TODT,106)VALID_UPTO,
				PIC_INTERVAL,SEQNO
				FROM PRODUCT_CAROUSEL_DISPLAY 
				WHERE ISDELETED=0 AND PDMID=$ddlProduct AND LOCID=$ddlLocation 
				ORDER BY SEQNO ASC";
		$data['query'] = $query;
		$count = unique($query);
		if($count>0){
...
function delete($mysqli){
	try{   
			...
			$data = array();     
            $PCID = ($_POST['PCID'] == 'undefined' || $_POST['PCID'] == '') ? 0 : $_POST['PCID'];  
			if($PCID == 0)throw new Exception('PCID Error.');
			$del = "EXEC [PRODUCT_CAROUSEL_DISPLAY_SP] 3,$PCID,0,0,'','','','','',0,0,$userid";
