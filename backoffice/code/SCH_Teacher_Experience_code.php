 ...
 */
 ...
 function save($mysqli){
     try
     {
		...
		$TEXT_INSTITUTION_NAME  = $_POST['TEXT_INSTITUTION_NAME'] == 'undefined' ? 0 : $_POST['TEXT_INSTITUTION_NAME'];
        $TEXT_POST_HELD  = $_POST['TEXT_POST_HELD'] == 'undefined' ? 0 : $_POST['TEXT_POST_HELD'];
		$TEXT_TEACHER_ID  = $_POST['TEXT_TEACHER_ID'] == 'undefined' ? 0 : $_POST['TEXT_TEACHER_ID'];
		$TEXT_GROSS_SALARY  = $_POST['TEXT_GROSS_SALARY'] == 'undefined' ? 0 : $_POST['TEXT_GROSS_SALARY'];
		...
		$actionid = $feesid == 0 ? 1 : 2;
				...
				$sql = "SELECT * FROM TEACHER_EXPERIENCE
		        WHERE TEACHER_EXPERIENCE_ID!=$feesid
	   ...
	   if($row_count == 0)
	   {
		...
		$query="EXEC [TEACHER_EXPERIENCE_SP] $actionid,$feesid,$TEXT_SCHOOL_ID,$TEXT_TEACHER_ID,$TEXT_INSTITUTION_NAME,$TEXT_POST_HELD,$TEXT_GROSS_SALARY,'$TEXT_DATE_OF_JOINING','$TEXT_DATE_OF_LEAVING',$userid,'$txtremarks' ";
 ...
 function getQuery($mysqli){
		try
	{
       ...
       $query =     "SELECT A.TEACHER_EXPERIENCE_ID
							,A.TEACHER_ID
							,A.SCHOOL_ID
							,A.SCHOOL_NAME
							,A.TEACHER_NAME
							...
							,A.POST_HELD
							,A.GROSS_SALARY
							,CONVERT(VARCHAR,A.DATE_OF_JOINING,106)DATE_OF_JOINING
							,CONVERT(VARCHAR,A.DATE_OF_LEAVING,106)DATE_OF_LEAVING
							,A.REMARKS
							from TEACHER_EXPERIENCE A , TEACHER B
							WHERE A.ISDELETED = 0
							AND   B.ISDELETED = 0
		...
		while ($row = sqlsrv_fetch_array($result,SQLSRV_FETCH_ASSOC)) {
			$row['TEACHER_EXPERIENCE_ID'] = (int) $row['TEACHER_EXPERIENCE_ID'];
			$data['data'][] = $row;
		}
		$data['success'] = true;
		echo json_encode($data);exit;
...
function delete($mysqli){
	try{   
			...
			if($feesid == 0){
				throw new Exception('TEACHER_EXPERIENCE_ID Error.');
			}
			
	
				$stmt=sqlsrv_query($mysqli, "EXEC [TEACHER_EXPERIENCE_SP]	3,$feesid,'','','','','','','',$userid,'' ") ;
