...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
    ...
    /* ========== GET STUDENT PAYMENT =========== */
    $scope.getStudentPayment = function () {
    ...
    $http({
        method: 'post',
        url: url,
        data: $.param({ 'type': 'getStudentPayment',
    ...
    then(function (data, status, headers, config) {
    //  console.log(data.data);
        $scope.post.getStudentPayment = data.data.success ? data.data.data : [];
        $scope.TOTAL_AMOUNT = data.data.success ? data.data.TOTAL_AMOUNT : 0;
        $('#SpinMainData').hide();
