...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
    ...
    $scope.PageSub1 = "LA_SUBJECT_M";
    
    var url = 'code/LA-SUBJECT-MASTER.php';
    ...
    $scope.save = function(){
        ...
        $http({
            ...
            transformRequest: function (data) {
                ...
                formData.append("subid", $scope.temp.subid);
     ...
     $scope.getSubjects = function () {
        // if(!$scope.temp.ddlLocation || $scope.temp.ddlLocation<=0) return;
         ...
         $http({
             method: 'post',
            url: url,
            // data: $.param({ 'type': 'getSubjects','ddlLocation':$scope.temp.ddlLocation}),
            data: $.param({ 'type': 'getSubjects','ddlLocation':1}),
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
        }).
        then(function (data, status, headers, config) {
            // console.log(data.data);
    ...
    $scope.edit = function (id) {
        $scope.temp = {
            ...
            txtSubject: id.SUBJECTNAME
        };
        $scope.editMode = true;
        $scope.index = $scope.post.getSubjects.indexOf(id);
    }
    ...
    $scope.delete = function (id) {
        ...
        if (r == true) {
            $http({
                method: 'post',
                url: url,
                data: $.param({ 'SUBID': id.SUBID, 'type': 'delete' }),
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            }).
		    then(function (data, status, headers, config) {
                //  console.log(data.data)
		        if (data.data.success) {
		            var index = $scope.post.getSubjects.indexOf(id);
		            $scope.post.getSubjects.splice(index, 1);
