 ...
 */
 ...
 function save($mysqli){
     try
     {
		...
		if($txtObject == '')
		{throw new Exception("Please Enter Object Function .");}

	$sql = "SELECT * FROM  MEP_OBJECT_FUNCTION_MASTER WHERE FUNCTION_DESC='$txtObject' AND FUNCTIONID!=$pmid AND ISDELETED=0 AND TECHPLATFORMID = $txTechPlatformId";
		$row_count = unique($sql);

		$data = array();
		if($row_count == 0)
		{
			$query="EXEC [IT_OBJECTFUNCTION_SP] $actionid,$pmid,'$txtObject',$userid ,$txTechPlatformId,'$txtremarks'";
...
/*============ Get PAYMENT MODES =============*/ 
 function getQuery($mysqli){
	try
	{
        ...
        $query = "SELECT FUNCTIONID, TECHPLATFORM, FUNCTION_DESC ,REMARKS FROM MEP_OBJECT_FUNCTION_MASTER WHERE ISDELETED=0";
...
function delete($mysqli){
	try{   
			...
			$stmt=sqlsrv_query($mysqli, "EXEC [IT_OBJECTFUNCTION_SP] 3,$pmid,'',$userid ,'',''");
			if( $stmt === false ) 
			{
