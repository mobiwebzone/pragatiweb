...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
    ...
    $scope.PageSub1 = "TRAINING_TRANSACTION";
    ...
    var url = 'code/Training_Transaction_code.php';
    ...
    $scope.init = function () {
        ...
        then(function (data, status, headers, config) {
            ...
            if (data.data.success) {
                ...
                $scope.temp.ddlLocation = data.data.locid;
                ...
                if($scope.userrole === 'SUPERADMIN'){
                    ...
                    $scope.temp.ddlTransaction = 'Teacher';
                    $scope.temp.ddlTeacher_Student = 'Admin';
                    $scope.getTeacher_Students();
                    $scope.temp.txtCompletedDT = new Date();
                }
                // window.location.assign("dashboard.html");

                $scope.PAGEFOR = $scope.userrole == 'SUPERADMIN' ? 'ADMIN' : 'TEACHER';
    ...
    $scope.chkTrnTrans_Proccess = false;
    $scope.getTrainingTransactions = function () {
        ...
        $scope.chkTrnTrans_Proccess = true;
    ...
    $scope.getTrainingMasters = function () {
        ...
        $http({
            method: 'post',
            url: 'code/Training_Master_code.php',
            data: $.param({ 
    ...
    $scope.getTeacher_Students = function () {
        ...
        $http({
            method: 'post',
            url: url,
            data: $.param({ 'type': 'getTeacher_Students','ddlTeacher_Student':$scope.temp.ddlTeacher_Student,'ddlLocation':$scope.temp.ddlLocation}),
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
        }).
        then(function (data, status, headers, config) {
            console.log(data.data);
    ...
    $scope.getLocations = function () {
        ...
        then(function (data, status, headers, config) {
            ...
            if($scope.temp.ddlLocation > 0) $scope.getTrainingTransactions();      
        },
        function (data, status, headers, config) {
            console.log('Failed');
        })
