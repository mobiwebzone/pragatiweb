...
function getStudents($mysqli){
	try
	{
		...
		if($ddlProduct > 0) $query .=" AND (SELECT PRODUCTID FROm STUDENT_COURSE_COVERAGE WHERE SCCID=SCCA.SCCID AND ISDELETED=0)=$ddlProduct";
		if($ddlInventory > 0) $query .=" AND (SELECT INVID FROm STUDENT_COURSE_COVERAGE WHERE SCCID=SCCA.SCCID AND ISDELETED=0)=$ddlInventory";
...
/*============ Get Student Test Report =============*/ 
 function getStudentReport($mysqli){
	try
	{
		...
		(SELECT FIRSTNAME+' '+LASTNAME FROM REGISTRATIONS WHERE REGID=SCCA.REGID)STUDENT
		FROM STUDENT_COURSE_COVERAGE_ATTENDEDBY SCCA WHERE
		(SELECT CONVERT(DATE,CDATE,105) FROM STUDENT_COURSE_COVERAGE WHERE SCCID=SCCA.SCCID AND ISDELETED=0) BETWEEN '$txtFromDT' AND '$txtToDT'";
			...
			while ($rowForALL = sqlsrv_fetch_array($resultForALL,SQLSRV_FETCH_ASSOC)) {
				...
				if($countDetails > 0){
					...
					while ($rowDetails = sqlsrv_fetch_array($resultDetails,SQLSRV_FETCH_ASSOC)) {
						...
						$grid .= '<td>'.$rowDetails['CDATE'].'</td>';
						$grid .= '<td>'.$rowDetails['CHAPTER'].'</td>';
						$grid .= '<td>'.$rowDetails['PAGEFROM'].'</td>';
						$grid .= '<td>'.$rowDetails['PAGETO'].'</td>';
						$grid .= '<td>'.$rowDetails['MAIN_REMARK'].'</td>';
						$grid .= '</tr>';
						// $grid .= '';
