...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
    ...
    $scope.Page = "PRODUCTS";
    $scope.PageSub = "PRO_CAROUSEL_MASTER";
    $scope.files = [];
    $scope.imageDimensionIsCorrect = false;
    $scope.imageHeight = 0;
    $scope.imageWidth = 0;
    
    var url = 'code/ProductCarouselMaster.php';
    ...
    $scope.save = function(){
        ...
        $http({
            ...
            transformRequest: function (data) {
                ...
                formData.append("pcid", $scope.temp.pcid);
     ...
     $scope.getCarouselMaster = function () {
        ...
        if((!$scope.temp.ddlProduct || $scope.temp.ddlProduct<=0) && (!$scope.temp.ddlLocation || $scope.temp.ddlLocation<=0))return;
        $('#SpinnerData').show();
        $http({
            method: 'POST',
            url: url,
    ...
    $scope.edit = function (id) {
        ...
        $scope.Img_src= id.PIC != '' ? 'images/product_carousel/'+id.PIC : '';
        $scope.editMode = true;
    ...
    $scope.delete = function (id) {
        ...
        if (r == true) {
            $http({
                method: 'post',
                url: url,
                data: $.param({ 'PCID': id.PCID, 'type': 'delete' }),
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            }).
		    then(function (data, status, headers, config) {
                // console.log(data.data)
    ...
    $scope.getProduct = function () {
        $http({
            method: 'post',
            url: masterUrl,
            data: $.param({ 'type': 'getProductMaster'}),
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
        }).
        then(function (data, status, headers, config) {
            // console.log(data.data);
