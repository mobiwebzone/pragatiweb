 ...
 */
 ...
 function CheckUserID($mysqli){
    try{
        ...
        $queryUser = "SELECT UID FROM USERS WHERE LOGINID='$txtLoginId' AND ISDELETED=0 AND (USERROLE='ADMINISTRATOR' OR USERROLE='USER' OR USERROLE='SUPERADMIN')";
        ...
        if($row_count > 0)
        {
            ...
            if($ret > 0)
            {
                $queryEmail = "SELECT FIRSTNAME,LASTNAME,DBO.GET_CLEAR_USER_PASSWORD($ret) AS PASS,
                EMAIL FROM USERS WHERE UID=$ret AND ISDELETED=0";
                ...
                sendMail($SEND_EMAIL,$USER_NAME,$PASS);
                $data['success'] = true;
                $data['EMAIL'] = $Email;
...
//  SEND EMAIL
function sendMail($SEND_EMAIL,$USER_NAME,$PASS){
