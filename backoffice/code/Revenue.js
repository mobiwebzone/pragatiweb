...
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CHANNEL SECTION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    $scope.saveData = function(){
        ...
        $http({
            ...
            transformRequest: function (data) {
                ...
                formData.append("ddlRevHead", $scope.temp.ddlRevHead);
                formData.append("ddlRevSubHead", $scope.temp.ddlRevSubHead);
    ...
    $scope.getRevenue = function () {
        $scope.post.getRevenue = [];
        if(!$scope.temp.ddlLocation || $scope.temp.ddlLocation<=0) return;
        $('#SpinMainData').show();
        $http({
            method: 'post',
            url: url,
            data: $.param({ 'type': 'getRevenue','ddlLocation':$scope.temp.ddlLocation}),
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
        }).
        then(function (data, status, headers, config) {
            // console.log(data.data);
            $scope.post.getRevenue = data.data.success?data.data.data:[];
            $('#SpinMainData').hide();
        },
    ...
    $scope.editForm = function (id) {
        ...
        $scope.temp = {
            ...
            txtDate : new Date(id.REVDATE),
            ddlRevHead : id.RHID.toString(),
            ddlRevSubHead : id.RSHID.toString(),
            txtAmount : Number(id.AMOUNT),
            ddlPaymentSource : id.REVFROM_BANKACCID.toString(),
            txtNarration : id.NARRATION,
            chkCapitalize : id.CAPITALIZE>0?'1':'0',
            chkAccrued : id.ACCRUED>0?'1':'0',
        }        
        $scope.editMode = true;
        $scope.index = $scope.post.getRevenue.indexOf(id);
    }
    /* ============ Edit Button ============= */ 
    ...
    $scope.delete = function (id) {
        ...
        if (r == true) {
		    ...
		    then(function (data, status, headers, config) {
                // console.log(data.data)
		        if (data.data.success) {
		            var index = $scope.post.getRevenue.indexOf(id);
		            $scope.post.getRevenue.splice(index, 1);
    ...
    $scope.getLocations = function () {
        ...
        then(function (data, status, headers, config) {
            ...
            if($scope.temp.ddlLocation > 0) $scope.getRevenue();
    ...
    $scope.getRevHeads = function () {
        $scope.post.getRevHeads = [];
        if(!$scope.temp.ddlLocation || $scope.temp.ddlLocation<=0) return;
        $('#SpinRevHead').show();
        $http({
            method: 'post',
            url: 'code/RevenueHeads.php',
    ...
    $scope.getRevSubHeads = function () {
        ...
        $http({
            method: 'post',
            url: 'code/RevenueHeads.php',
            data: $.param({ 'type': 'getRevSubHeads', 'rhid' : $scope.temp.ddlRevHead}),
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
        }).
        then(function (data, status, headers, config) {
            // console.log(data.data);
