...
if( isset($_POST['type']) && !empty($_POST['type'] ) ){
	...
	switch ($type) {
        case "getFreeTestUsersList":getFreeTestUsersList($conn);break;
        case "getFreeTests":getFreeTests($conn);break;
        case "deleteFreeTestUser":deleteFreeTestUser($conn);break;
...
/*============ GET FREE TEST USERS LIST =============*/ 
function getFreeTestUsersList($mysqli){
	try
	{
		$data = array();
		$query = "SELECT REGID,CONVERT(VARCHAR,INSERTDATE,106)TESTDATE,FIRSTNAME,LASTNAME,GRADE,PHONE,EMAIL FROM FREE_TEST_USERS WHERE ISDELETED=0 ORDER BY CONVERT(DATETIME,INSERTDATE,20) DESC";
		...
		if($count > 0){
			while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
				$REGID = $row['REGID'];
				// GET TESTS
				$qryTest = "SELECT TESTID,(SELECT TESTDESC FROM TEST_MASTER WHERE TESTID=STF.TESTID)TEST 
				FROM STUDENT_TESTS_FREE STF WHERE REGID=$REGID AND ISDELETED=0 GROUP BY TESTID";
				$cntTest = unique($qryTest);
				if($cntTest>0){
					$resTest = sqlsrv_query($mysqli, $qryTest);
					while ($rowTest = sqlsrv_fetch_array($resTest, SQLSRV_FETCH_ASSOC)) {
						$row['TEST_LIST'][] = $rowTest;
					}
				}else{
					$row['TEST_LIST'] = array();
				}
...
/*============ GET FREE TESTS =============*/ 
 function getFreeTests($mysqli){
	try
	{
		$data = array();

		$query = "SELECT TESTID,TESTDESC FROM TEST_MASTER WHERE FREE_TEST=1 AND ISDELETED=0 ORDER BY TESTDESC";
		$result = sqlsrv_query($mysqli, $query);
		$count = unique($query);
		if($count > 0){
			while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
			...
			$data['message'] = 'Free test not found.';
			$data['success'] = false;
		}
		$data['$query'] = $query;
		echo json_encode($data);exit;
...
function getTestSection($mysqli){
	try
	{
		...
		FROM STUDENT_TESTS_FREE ST WHERE TESTID=$ddlTest --AND ATTEMPT=1 
...
/*============ GET TEST SECTION ATTEMPTS =============*/ 
 function getTestSectionAttempts($mysqli){
	try
	{
		...
		$query = "SELECT COUNT(*)[COUNT],ATTEMPT FROM STUDENT_TESTS_FREE 
...
//* =============== DELETE ATTEMPT ==============*//
function deleteFreeTestUser($mysqli){
	try{   
			global $userid;
			$data = array();     
            $REGID = ($_POST['REGID'] == 'undefined' || $_POST['REGID'] == '') ? 0 : $_POST['REGID'];
			if($REGID == 0) throw new Exception('REGID Error.');
			$delQuery = "UPDATE FREE_TEST_USERS SET ISDELETED=1,DELETEID=$userid,DELETEDATE=GETDATE() WHERE REGID=$REGID; 
						UPDATE STUDENT_TESTS_FREE SET ISDELETED=1 WHERE REGID=$REGID;
						UPDATE STUDENT_TEST_RESULTS_FREE SET ISDELETED=1 WHERE REGID=$REGID";
...
/*============ GET STUDENT BY PLAN =============*/ 
 function getStudentByPlan($mysqli){
	try
	{
		...
		AND REGID IN (SELECT REGID FROM REGISTRATIONS WHERE LOCATIONID=$LOCID)";
...
/*============ GET STUDENT ANSWERS =============*/ 
 function getStudentAnswer($mysqli){
	try
	{
		...
		(SELECT TESTSECTION FROM TEST_SECTIONS WHERE TSECID = ST.TSECID AND ISDELETED=0)SECTION,
		(SELECT CONVERT(VARCHAR,TESTDATE,106) FROM STUDENT_TESTS_FREE WHERE STID=ST.STID AND ATTEMPT=$ATTEMPT AND ISDELETED=0)TESTDATE,
		(SELECT CONVERT(VARCHAR(15),CAST(CONVERT(VARCHAR(8),STARTDATETIME,108) AS TIME),100) FROM STUDENT_TESTS_FREE WHERE STID=ST.STID AND ATTEMPT=$ATTEMPT AND ISDELETED=0)STARTDATETIME,
		(SELECT CONVERT(VARCHAR(15),CAST(CONVERT(VARCHAR(8),ENDDATETIME,108) AS TIME),100) FROM STUDENT_TESTS_FREE WHERE STID=ST.STID AND ATTEMPT=$ATTEMPT AND ISDELETED=0)ENDDATETIME
		FROM STUDENT_TEST_RESULTS_FREE ST
		WHERE ISDELETED=0 AND REGID=$REGID AND TSECID=$TSECID AND
		STID=(SELECT STID FROM STUDENT_TESTS_FREE WHERE ATTEMPT=$ATTEMPT AND ISDELETED=0 AND
...
function DeleteAttempt($mysqli){
	try{   
			...
			$delQuery = "UPDATE STUDENT_TESTS_FREE SET ISDELETED=1 WHERE STID=$STID; 
						UPDATE STUDENT_TEST_RESULTS_FREE SET ISDELETED=1 WHERE STID=$STID";
			$stmt=sqlsrv_query($mysqli, $delQuery);
			if( $stmt === false ) 
			{
				die( print_r( sqlsrv_errors(), true));
