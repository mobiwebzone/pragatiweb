...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout,$window) {
    ...
    $scope.PageSub = "STUDENT_COLLEGE_ROADMAP";
    $scope.editMode = false;
    
    var url = 'code/Student_College_Roadmap.php';
...
/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% SAVE MASTERS START %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */ 
    ...
    $scope.saveData = function(){
        ...
        $http({
            ...
            transformRequest: function (data) {
                ...
                formData.append("BackOff",$scope.temp.BackOff);
                return formData;
            },
            data: $scope.temp,
        ...
        then(function (data, status, headers, config) {
            ...
            if (data.data.success) {
                ...
                $scope.getStudentCollegeRoadmap();
                // if($scope.temp.roadmapid > 0) $scope.getStudentApplications_DET(); 
    ...
    $scope.getStudentCollegeRoadmap = function () {
        ...
        $http({
             method: 'post',
             url: url,
            data: $.param({ 'type': 'getStudentCollegeRoadmap','roadmapid':0,'regid':0}),
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
        }).
        then(function (data, status, headers, config) {
            // console.log(data.data);
    ...
    $scope.deleteData = function (id) {
        ...
        if (r == true) {
            $http({
                method: 'post',
                url: url,
                data: $.param({ 'ROADMAPID': id.ROADMAPID, 'type': 'deleteData' }),
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            }).
		    then(function (data, status, headers, config) {
                // console.log(data.data)
		        if (data.data.success) {
		            var index = $scope.post.getStudentCollegeRoadmap.indexOf(id);
		            $scope.post.getStudentCollegeRoadmap.splice(index, 1);
...
/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% SAVE ACTIVITY START %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */ 
    ...
    $scope.saveDataActivity = function(){
        alert($scope.temp.ddlActivity_Activity);
        // console.log($scope.ACTIVITY_model);
...
/* ######################################################################################################################### */
    ...
    $scope.getStudentByLoc = function () {
        ...
        then(function (data, status, headers, config) {
            ...
            if(data.data.success){
                ...
                if(!$scope.editMode)if($scope.editMode)$timeout(()=>{if($scope.temp.ddlStudent>0)$scope.setStudentDetails()},500);
            }else{
                $scope.post.getStudentByLoc = [];
            }
            $('.spinUser').hide();
   ...
   /* ========== GET CITY =========== */
    ...
    $scope.getActivities = function () {
        if(!$scope.post.getActivities || $scope.post.getActivities.length<=0){
            ...
            $http({
                method: 'post',
                url: 'code/Activities_Master.php',
   ...
   /* ========== GET COLLEGES =========== */
    ...
    /* ========== OPEN PRINT ROADMAP PAGE =========== */
    $scope.OpenPrint = function (id){
        if(id.REGID <=0){alert('Student Id Not Found.'); return;}

        // $window.location.href = '../backoffice/Receipt/College_Roadmap_Print.html?REGID='+id.REGID+'';
        // window.location.assign('../backoffice/Receipt/College_Roadmap_Print.html?REGID='+id.REGID+'','_blank');
        window.open('../backoffice/Receipt/College_Roadmap_Print.html?REGID='+id.REGID+'','_blank');
        console.log(id);
    }
    /* ========== OPEN PRINT ROADMAP PAGE =========== */
