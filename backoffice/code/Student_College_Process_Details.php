		...
		// {
			if($actionid == 1)	{
				...
				if($stmt){
					...
					if($FROM == 'ST'){
						$updquery = "UPDATE STUDENT_COLLEGE_PROCESS_STEPS_DETAILS SET STEP_STATUS='Close' WHERE SCPSDID=$scpsdid";
					}else{
						$updquery = "UPDATE STUDENT_COLLEGE_PROCESS_STEPS_DETAILS SET STEP_STATUS='Close' WHERE SCPSDID=$scpsdid";
					}
	...
	try
	{
		...
		(SELECT COLLEGE FROM COLLEGES_MASTER 
			WHERE CLID=(SELECT CLID FROM STUDENT_COLLEGE_PROCESS 
				...
				(SELECT STEPNO FROM STEPS_MASTER WHERE STEPID=SCPSD.STEPID)STEPNO
