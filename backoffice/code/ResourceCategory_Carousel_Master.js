...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
    ...
    $scope.Page = "FREERES";
    $scope.PageSub = "RES_CAROUSEL_MASTER";
    $scope.files = [];
    $scope.imageDimensionIsCorrect = false;
    $scope.imageHeight = 0;
    $scope.imageWidth = 0;
    
    var url = 'code/ResourceCategory_Carousel_Master.php';
    ...
    $scope.save = function(){
        ...
        $http({
            ...
            transformRequest: function (data) {
                ...
                formData.append("rccid", $scope.temp.rccid);
     ...
     $scope.getCarouselMaster = function () {
        ...
        if(!$scope.temp.ddlResCat || $scope.temp.ddlResCat<=0)return;
        $('#SpinnerData').show();
        $http({
            method: 'POST',
            url: url,
    ...
    $scope.edit = function (id) {
        ...
        $scope.Img_src= id.PIC != '' ? 'images/resource_category_carousel/'+id.PIC : '';
        $scope.editMode = true;
    ...
    $scope.delete = function (id) {
        ...
        if (r == true) {
            $http({
                method: 'post',
                url: url,
                data: $.param({ 'RCCID': id.RCCID, 'type': 'delete' }),
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            }).
		    then(function (data, status, headers, config) {
                // console.log(data.data)
    ...
    $scope.getResourceCategories = function () {
        $http({
            method: 'post',
            url: url,
            data: $.param({ 'type': 'getResourceCategories'}),
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
        }).
        then(function (data, status, headers, config) {
            // console.log(data.data);
            $scope.post.getResourceCategories = data.data.data;
        },
        function (data, status, headers, config) {
            console.log('Failed');
        })
    }
    // $scope.getResourceCategories(); --INIT
