...
/*============ Get Report =============*/ 
 function getReport($mysqli){
	try
	{
		...
		$query = "EXEC [RPT_TUTORING_TEACHERS_DUES] $ddlLocation,$userid,'$ddlPaidDue',$sum_det";
		$data['query']=$query;
		...
		if(sqlsrv_has_rows($result) !== false){
			...
			while ($row = sqlsrv_fetch_array($result,SQLSRV_FETCH_ASSOC)) {
				...
				$row['TEACHER_SHARE'] = (float) $row['TEACHER_SHARE'];
				$row['DUE'] = (float) ($row['TEACHER_SHARE']-$row['PAID']);
				$data['data'][] = $row;
				$TOTAL_AMT += $row['AMOUNT'];
				$TOTAL_PAID += $row['PAID'];
				$TOTAL_TS += $row['TEACHER_SHARE'];
				$TOTAL_DUE += $row['DUE'];
			}
			$data['TOTALS'] = compact('TOTAL_AMT','TOTAL_PAID','TOTAL_TS','TOTAL_DUE');
			$data['success'] = true;
		}else{
			$data['success'] = false;
		}
