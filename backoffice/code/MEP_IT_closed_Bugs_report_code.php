...
if( isset($_POST['type']) && !empty($_POST['type'] ) ){
	...
	switch ($type) {
        ...
        case "UpdateTodo":UpdateTodo($conn);break;
...
/* =============== UPDATE TODO ==================*/
 function UpdateTodo($mysqli){
     try
     {
		...
		if($txtTodoUpd == ''){throw new Exception("Please Enter 'Todo'.");}
		...
		else
		{
			$data['query'] = $query;
			$data['success'] = true;
			$data['message'] = 'Todo successfully updated.';
			echo json_encode($data);exit;
		}
...
/*============ Get TODO =============*/ 
 function getTODO($mysqli){
	try
	{
		...
		$query = "SELECT (SELECT [LOCATION] FROM LOCATIONS WHERE LOC_ID=TDL.LOCID)[LOCATION],
		(SELECT FIRSTNAME+' '+LASTNAME FROM USERS WHERE [UID]=TDL.TOUSER)[USER],
		TODO,
		CONVERT(VARCHAR,UPDATEDATE,106) Updation_Date,
		CONVERT(VARCHAR,ETA,106)ETA
		FROM TO_DO_LIST TDL WHERE ISDELETED=0 
		AND TDSSUBCATID  in (226,229) AND TDSTATUS='CLOSED'";
