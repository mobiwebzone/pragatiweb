...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout) {
    ...
    var url = '../backoffice/code/HourlyAttendance.php';
    ...
    $scope.init = function () {
        ...
        then(function (data, status, headers, config) {
            ...
            if (data.data.success) {
                ...
                if($scope.LOC_ID > 0) $scope.getAtt();
                if($scope.LOC_ID > 0) $scope.getStudentByTeacher();
            }
            else {
    ...
    $scope.Save = function(){        
        //alert($scope.temp.txtTimeIN.toLocaleTimeString('sv-SE'));
        ...
        $http({
            ...
            transformRequest: function (data) {
                ...
                formData.append("ddlTeacher", $scope.userid);
    ...
    $scope.getStudentByTeacher = function () {
        ...
        $scope.selectedStudent=[];
        $scope.temp.studentcheck={};
        if(!$scope.userid || $scope.userid<=0)return;
        $scope.spinSubject =  true;
        $http({
            method: 'post',
            url: url,
            data: $.param({ 'type': 'getStudentByTeacher','ddlTeacher':$scope.userid,'REGID':$scope.GET_REGID}),
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
        }).
        then(function (data, status, headers, config) {
            // console.log(data.data);
