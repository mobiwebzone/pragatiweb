...
$postModule.controller("myCtrl", function ($scope, $http,$interval,$timeout,taOptions) {
    ...
    var url = '../student_zone/code/LearnAssis.php';
    ...
    $scope.getGradeSubject = function () {
         ...
         $http({
             method: 'post',
             url: url,
             data: $.param({ 'type': 'getGradeSubject','ID':$scope.userid,'FOR':'TEACHER'}),
         headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
     }).
     then(function (data, status, headers, config) {
         // console.log(data.data);
